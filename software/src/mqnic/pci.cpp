...
void remove_driver(const char* pci_addr) {
    ...
    log_error("There should be no drivers!");
    // Or unload using the following code
...
void enable_dma(const char* pci_addr) {
    ...
    dma |= 1 << 2;
    assert(lseek(fd, 4, SEEK_SET) == 4);
    assert(write(fd, &dma, 2) == 2); // TODO: uncomment this
    check_err(close(fd), "close");
    info("dma enabled");
}

uint8_t* pci_map_resource(const char* pci_addr, long long unsigned int* size) {
    char path[PATH_MAX];
    ...
    info("pci resource size: %ldB", stat.st_size);
    *size = stat.st_size;
